...
class WfTrackingUtil {
	...
	public static function get_shipment_display_default_message( $shipment_result_array ) {
		...
		if ( isset( $shipment_result_array['tracking_info'] ) ) {
			...
			$sub_message_1 = __(' To track shipment, please follow the shipment', 'wf-shipping-dhl');
			$sub_message_2 = '';
	...
	public static function update_tracking_data ( $order_id, 
		...
		$shipment_source_data = $order->get_meta($shipment_source_key);
		...
		$order->update_meta_data($shipment_source_key, $shipment_source_data);
		...
		if ( null != $shipment_result && is_object( $shipment_result ) ) {
		    ...
		    $order->update_meta_data($shipment_result_key, $shipment_result_array);
	    	...
	    	$order->update_meta_data($shipment_result_key, '');
