...
class WF_Tracking_Admin_DHLExpress {
	...
	function __construct() {
		...
		if ($this->add_trackingpin_shipmentid === 'yes' && !ELEX_DHL_EXPRESS_AUTO_LABEL_GENERATE_ADDON_WOOCOMMERCE_EXTENSION) {
...
XML;
	...
	function wf_admin_notice() {
		global $pagenow;
		global $post;
		if (!$post && (isset($_GET['id']) && empty( $_GET['id']) )) {
            return;
        }
        if( isset( $_GET['id'] ) ){
	...
	<?php   $order = wc_get_order($post->ID);
			$saved_tracking_data = $order->get_meta('_dhl_tracking_shipment_tracking_data');
