...
class wf_dhl_paket_woocommerce_shipping_admin_helper {
	...
	private $return_account_number;
	private $site_id;
	private $site_password;
	private $api_user;
	private $api_key;
	private $api_url;
	private $soap_header_url;
	private $service_url;
	private $debug;
	...
	private function init() {       
		...
		$this->production    = ( $bool = $this->settings[ 'production' ] ) && 'yes' ==  $bool ? true : false;
		...
		$this->debug                  = ( $bool = $this->settings[ 'debug' ] ) && 'yes' == $bool ? true : false;
		$this->insure_contents        = ( $bool = @$this->settings[ 'insure_contents' ] ) && 'yes' == $bool ? true : false;
	...
	private function wf_get_shipment_details( $dhl_packages, $params = array()) {
		...
		if ($this->origin_country == 'DE' && $this->destination_country !='DE') {
			...
			if ($this->is_european_country($this->destination_country)) {
				...
				} elseif ($this->dhl_connect_enabled ) {//Paket Connect
	...
	public function generate_packages( $order, $service_code, $auto_label = '') {
		...
		if ( is_array( $package ) ) {
			...
			if ($auto_label == 'true') {
				...
				$order->update_meta_data('_wf_dhl_paket_stored_packages', $dhl_packages);
				$order->save();
	...
	private function process_result( $result, $request, $dhl_packages, $auto_label ) {
		...
		if (!empty($result['ShipmentID']) && !empty($result['LabelImage'])) {            
			...
			$order->update_meta_data( '_wf_woo_dhl_shipmentId', $stored_shipments_ids, false);
			$order->update_meta_data( '_wf_woo_dhl_shippingLabel_'. $shipmentId, $shippingLabel);
			$order->update_meta_data( '_wf_woo_dhl_piece_information', $pieceInformation);
			$order->save();
