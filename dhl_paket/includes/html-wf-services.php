<tr valign="top" id="service_options">
	...
	<strong><?php _e( 'Services', 'wf-shipping-dhl' ); ?></strong><br><br>
				...
				foreach ( $this->ordered_services as $value ) {
					...
					?>
						<tr>
							...
							<td><input type="hidden" name="dhl_service[<?php echo $code; ?>][name]" placeholder="<?php echo $name; ?>" value="<?php echo isset( $this->custom_services[ $code ]['name'] ) ? $this->custom_services[ $code ]['name'] : ''; ?>" size="50" /></td>
							<td><input type="checkbox" name="dhl_service[<?php echo $code; ?>][enabled]" <?php checked( ( ! isset( $this->custom_services[ $code ]['enabled'] ) || ! empty( $this->custom_services[ $code ]['enabled'] ) ), true ); ?> /></td>
							<!--
