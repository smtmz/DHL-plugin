...
class WF_Boxpack_Box {
	...
	public function pack( $items ) {
		...
		while ( sizeof( $items ) > 0 ) {
			...
			// Check max weight
			if ( ( $packed_weight + (float) $item->get_weight() ) > $this->get_max_weight() && $this->get_max_weight() > 0 ) {
				$unpacked[] = $item;
				continue;
			}
		...
		$packed_weight       = (float) $packed_weight;
		$unpacked_weight     = (float) $unpacked_weight;
		...
		$packed_weight_ratio = (float) $packed_weight_ratio;
		$packed_volume_ratio = (float) $packed_volume_ratio;
