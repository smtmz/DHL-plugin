...
class WF_Tracking_Admin_DHLParcel {

	const SHIPPING_METHOD_DISPLAY = 'Tracking';
	const TRACKING_TITLE_DISPLAY  = 'DHL Parcel Shipment Tracking';

	const TRACK_SHIPMENT_KEY   = 'wf_dhlparcel_shipment'; //Note: If this key is getting changed, do the same change in JS code below.
	const SHIPMENT_SOURCE_KEY  = 'wf_dhlparcel_shipment_source';
	const SHIPMENT_RESULT_KEY  = 'wf_dhlparcel_shipment_result';
	const TRACKING_MESSAGE_KEY = 'wfdhlparceltrackingmsg';
	const TRACKING_METABOX_KEY = 'WF_Tracking_Metabox_DHLParcel';
...
return;
	...
	function wf_tracking_metabox_content() {
		...
		<ul class="order_actions submitbox">
			<li id="actions" class="wide">
				<select name="shipping_service_dhlparcel" id="shipping_service_dhlparcel">
	<?php
				...
				//foreach ( $this->tracking_data as $key => $details ) {
					...
					echo '<option value=' . 'deutsche-post-dhl' . ' ' . selected($shipment_source_data['shipping_service'], 'deutsche-post-dhl') . ' >' . __( 'DHL Parcel', 'wf-shipping-dhl' ) . '</option>';
				//}
		?>
				</select><br>
				...
				<textarea id="tracking_dhlparcel_shipment_ids" class="input-text" type="text" name="tracking_dhlparcel_shipment_ids" ><?php echo $shipment_source_data['shipment_id_cs']; ?></textarea><br>
				...
				<input type="text" id="order_date_dhlparcel" class="date-picker" value="<?php echo $shipment_source_data['order_date']; ?>"></p>
			</li>
			<li id="" class="wide">
				<a class="button button-primary woocommerce_shipment_dhlparcel_tracking tips" href="<?php echo $tracking_url; ?>" data-tip="<?php _e( 'Save/Show Tracking Info', 'wf-shipping-dhl' ); ?>"><?php _e('Save/Show Tracking Info', 'wf-shipping-dhl'); ?></a>
			</li>
		</ul>
		<script>
			...
			jQuery("a.woocommerce_shipment_dhlparcel_tracking").on("click", function() {
			   location.href = this.href + '&wf_dhlparcel_shipment=' + jQuery('#tracking_dhlparcel_shipment_ids').val().replace(/ /g,'')+'&shipping_service='+ jQuery( "#shipping_service_dhlparcel" ).val()+'&order_date='+ jQuery( "#order_date_dhlparcel" ).val();
			   return false;
			});
		</script>
	<?php
	...
	function wf_display_admin_track_shipment() {
		...
		$dhl_parcel_settings = get_option( 'woocommerce_' . WF_DHL_PARCEL_ID . '_settings', null );
		if ( !empty( $dhl_parcel_settings['custom_message'] ) ) {
			update_option( WfTrackingUtil::TRACKING_SETTINGS_TAB_KEY . WfTrackingUtil::TRACKING_MESSAGE_KEY, $dhl_parcel_settings['custom_message'] );
		} else {
...
new WF_Tracking_Admin_DHLParcel();
